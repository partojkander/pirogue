...
class Room:
    ...
    def draw(self, screen : window) -> None:
        ...
        for i in range(self.y_offset+1, self.y_offset + self.height-1):
            screen.addstr(i, self.x_offset, self.wall + self.floor * (self.width-2) + self.wall)
        screen.addstr(self.y_offset + self.height-1, self.x_offset, self.wall * self.width)
    ...
    def can_move_to(self, x, y) -> bool:
        if not self.is_inside_room(x, y):
            ...
            return True
        if y == self.y_offset or y == self.y_offset + self.height-1:  # In top or bottom wall
            return False
